...
let db = {
  ...
  players: {},
  staff: [
    { id: 's1', name: 'Nguyễn Văn A', role: 'Dealer', status: 'offline', total_minutes: 0 },
    { id: 's2', name: 'Trần Thị B', role: 'Floor', status: 'offline', total_minutes: 0 },
    { id: 's3', name: 'Lê Văn C', role: 'Dealer', status: 'offline', total_minutes: 0 },
    { id: 's4', name: 'Phạm Thị D', role: 'Thu ngân', status: 'offline', total_minutes: 0 },
    { id: 's5', name: 'Hoàng Văn E', role: 'TD', status: 'offline', total_minutes: 0 }
  ],
  members: []
};
...
io.on('connection', (socket) => {
  ...
  socket.on('staff_checkin', (staffId) => {
    const s = db.staff.find(s => s.id === staffId);
    if (s && s.status === 'offline') { s.status = 'waiting'; broadcastState(); }
  });
